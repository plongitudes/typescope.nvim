-- The resolver (design/oracle.md §5): function_scope, recurse, clear_cache,
-- answered by one `typescope/structure` request to the oracle and an adapter
-- from the wire Scope to the Node trees the float draws.
--
-- Behaviors carried over from the treesitter resolver this replaced, by
-- name, so a reader can check each was kept rather than lost with the code
-- that implemented it:
...
--- Wire Node → model spec. `evaluated` is the ≈ text the renderer draws:
--- an inferred type (with the declared type reading "Any", which the
