...
## 2026-09-21 — tick 13 — `upm` footprint-final

- Release build (`scripts/build-oracle.sh --release`, 2m29s cold, 27 MB binary). `scripts/footprint.lua` (kept; it is the measurement the changelog cites) opens the float through the plugin on the ten kitchen targets with real basedpyright + the release oracle attached, then `footprint(1)` on both trees: **basedpyright 528 MB settled, oracle 154 MB settled (88 at attach)**; first open 1.7 s, then 5–130 ms.
